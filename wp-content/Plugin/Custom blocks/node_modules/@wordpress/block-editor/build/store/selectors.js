...
const canInsertBlockTypeUnmemoized = (state, blockName, rootClientId = null) => {
  ...
  const _isSectionBlock = !!(0, _privateSelectors.isSectionBlock)(state, rootClientId);
  if (_isSectionBlock) {
    return false;
  }
 ...
 * @return {boolean} Whether the given block type is allowed to be inserted.
 */
const canInsertBlockType = exports.canInsertBlockType = (0, _data.createSelector)(canInsertBlockTypeUnmemoized, (state, blockName, rootClientId) => (0, _utils.getInsertBlockTypeDependants)(state, rootClientId));
...
const getInserterItems = exports.getInserterItems = (0, _data.createRegistrySelector)(select => (0, _data.createSelector)((state, rootClientId = null, options = DEFAULT_INSERTER_OPTIONS) => {
  ...
  const syncedPatternInserterItems = canInsertBlockTypeUnmemoized(state, 'core/block', rootClientId) ? (0, _lockUnlock.unlock)(select(_constants.STORE_NAME)).getReusableBlocks().map(buildReusableBlockInserterItem) : [];
...
}, (state, rootClientId) => [(0, _blocks.getBlockTypes)(), (0, _lockUnlock.unlock)(select(_constants.STORE_NAME)).getReusableBlocks(), state.blocks.order, state.preferences.insertUsage, ...(0, _utils.getInsertBlockTypeDependants)(state, rootClientId)]));
 ...
 * @property {number}          frecency     Heuristic that combines frequency and recency.
 */
const getBlockTransformItems = exports.getBlockTransformItems = (0, _data.createSelector)((state, blocks, rootClientId = null) => {
...
}, (state, blocks, rootClientId) => [(0, _blocks.getBlockTypes)(), state.preferences.insertUsage, ...(0, _utils.getInsertBlockTypeDependants)(state, rootClientId)]);
 ...
 */
const hasInserterItems = exports.hasInserterItems = (0, _data.createRegistrySelector)(select => (state, rootClientId = null) => {
  const hasBlockType = (0, _blocks.getBlockTypes)().some(blockType => canIncludeBlockTypeInInserter(state, blockType, rootClientId));
  if (hasBlockType) {
    return true;
  }
  const hasReusableBlock = canInsertBlockTypeUnmemoized(state, 'core/block', rootClientId) && (0, _lockUnlock.unlock)(select(_constants.STORE_NAME)).getReusableBlocks().length > 0;
...
const getAllowedBlocks = exports.getAllowedBlocks = (0, _data.createRegistrySelector)(select => (0, _data.createSelector)((state, rootClientId = null) => {
  ...
  return blockTypes;
}, (state, rootClientId) => [(0, _blocks.getBlockTypes)(), (0, _lockUnlock.unlock)(select(_constants.STORE_NAME)).getReusableBlocks(), ...(0, _utils.getInsertBlockTypeDependants)(state, rootClientId)]));
...
const getAllowedPatternsDependants = select => (state, rootClientId) => [...(0, _utils.getAllPatternsDependants)(select)(state), ...(0, _utils.getInsertBlockTypeDependants)(state, rootClientId)];
...
const __experimentalGetAllowedPatterns = exports.__experimentalGetAllowedPatterns = (0, _data.createRegistrySelector)(select => {
  return (0, _data.createSelector)((state, rootClientId = null, options = DEFAULT_INSERTER_OPTIONS) => {
    ...
    const patternsAllowed = availableParsedPatterns.filter(pattern => (0, _utils.getGrammar)(pattern).every(({
      blockName: name
    }) => options[_utils.isFiltered] !== false ? canInsertBlockType(state, name, rootClientId) : isBlockVisibleInTheInserter(state, name)));
 ...
 */
const getBlockEditingMode = exports.getBlockEditingMode = (0, _data.createRegistrySelector)(select => (state, clientId = '') => {
