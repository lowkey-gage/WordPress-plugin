...
exports.__unstableSetEditorMode = void 0;
exports.__unstableSetTemporarilyEditingAsBlocks = __unstableSetTemporarilyEditingAsBlocks;
...
exports.hideInsertionPoint = exports.flashBlock = void 0;
exports.hoverBlock = hoverBlock;
exports.insertBeforeBlock = exports.insertAfterBlock = void 0;
exports.insertBlock = insertBlock;
exports.mergeBlocks = exports.insertDefaultBlock = exports.insertBlocks = void 0;
...
const validateBlocksToTemplate = blocks => ({
  ...
  const isBlocksValidToTemplate = !template || templateLock !== 'all' || (0, _blocks.doBlocksMatchTemplate)(blocks, template);
...
const multiSelect = (start, end, __experimentalInitialPosition = 0) => ({
  ...
  (0, _a11y.speak)((0, _i18n.sprintf)( /* translators: %s: number of selected blocks */
...
const moveBlocksDown = exports.moveBlocksDown = createOnMove('MOVE_BLOCKS_DOWN');
const moveBlocksUp = exports.moveBlocksUp = createOnMove('MOVE_BLOCKS_UP');
...
const __unstableDeleteSelection = isForward => ({
  ...
  valueB = (0, _richText.insert)(valueB, _selection.START_OF_SELECTED_AREA, 0, selectionB.offset);
  ...
  const blocksWithTheSameType = blockA.name === blockB.name ? [followingBlock] : (0, _blocks.switchToBlockType)(followingBlock, targetBlockType.name);
  ...
  const newAttributeKey = (0, _selection.retrieveSelectedAttribute)(updatedAttributes);
  const convertedHtml = updatedAttributes[newAttributeKey];
  const convertedValue = (0, _richText.create)({
    html: convertedHtml
  });
  const newOffset = convertedValue.text.indexOf(_selection.START_OF_SELECTED_AREA);
...
const __unstableSplitSelection = (blocks = []) => ({
  ...
  const attributeKeyA = typeof selectionA.attributeKey === 'string' ? selectionA.attributeKey : (0, _selection.findRichTextAttributeKey)(blockAType);
  const attributeKeyB = typeof selectionB.attributeKey === 'string' ? selectionB.attributeKey : (0, _selection.findRichTextAttributeKey)(blockBType);
  ...
  if (bindings?.[attributeKeyA]) {
    ...
    if (blocks.length) {
      ...
      createWarningNotice((0, _i18n.__)("Blocks can't be inserted into other blocks with bindings"), {
        type: 'snackbar'
      });
      return;
    }
  ...
  if (selectionA.clientId === selectionB.clientId && attributeKeyA === attributeKeyB && selectionA.offset === selectionB.offset) {
    ...
    // empty block, provided this block has no inner blocks.
      ...
      function createEmpty() {
        ...
        return select.canInsertBlockType(defaultBlockName, anchorRootClientId) ? (0, _blocks.createBlock)(defaultBlockName) : (0, _blocks.createBlock)(select.getBlockName(selectionA.clientId));
      }
  ...
  let head = {
    ...
    attributes: {
      ...blockA.attributes,
      [attributeKeyA]: (0, _richText.toHTMLString)({
        value: valueA
      })
    }
  };
  let tail = {
    ...
    clientId: blockA.clientId === blockB.clientId ? (0, _blocks.createBlock)(blockB.name).clientId : blockB.clientId,
    attributes: {
      ...blockB.attributes,
      [attributeKeyB]: (0, _richText.toHTMLString)({
        value: valueB
      })
    }
  };
  ...
  blockA.clientId === blockB.clientId && defaultBlockName && tail.name !== defaultBlockName && select.canInsertBlockType(defaultBlockName, anchorRootClientId)) {
    const switched = (0, _blocks.switchToBlockType)(tail, defaultBlockName);
  ...
  const headType = (0, _blocks.getBlockType)(head.name);
  const firstBlocks = headType.merge && firstBlock.name === headType.name ? [firstBlock] : (0, _blocks.switchToBlockType)(firstBlock, headType.name);
  if (firstBlocks?.length) {
    ...
    selection = {
      ...
      offset: (0, _richText.create)({
        html: head.attributes[attributeKeyA]
      }).text.length
    };
    clonedBlocks.unshift(...firstBlocks);
  } else {
    if (!(0, _blocks.isUnmodifiedBlock)(head)) {
      output.push(head);
    }
    output.push(firstBlock);
  }
  const lastBlock = clonedBlocks.pop();
  const tailType = (0, _blocks.getBlockType)(tail.name);
  if (clonedBlocks.length) {
    output.push(...clonedBlocks);
  }
  if (lastBlock) {
    const lastBlocks = tailType.merge && tailType.name === lastBlock.name ? [lastBlock] : (0, _blocks.switchToBlockType)(lastBlock, tailType.name);
    if (lastBlocks?.length) {
      ...
      if (!(0, _blocks.isUnmodifiedBlock)(tail)) {
        output.push(tail);
      }
    }
  } else if (!(0, _blocks.isUnmodifiedBlock)(tail)) {
    output.push(tail);
  }
  registry.batch(() => {
...
const mergeBlocks = (firstBlockClientId, secondBlockClientId) => ({
  ...
  if (canRestoreTextSelection) {
    ...
    const value = (0, _richText.insert)((0, _richText.create)({
      html
    }), _selection.START_OF_SELECTED_AREA, offset, offset);
  ...
  // We can only merge blocks with similar types
  // thus, we transform the block to merge first.
  const blocksWithTheSameType = blockA.name === blockB.name ? [cloneB] : (0, _blocks.switchToBlockType)(cloneB, blockA.name);
...
const insertDefaultBlock = (attributes, rootClientId, index) => ({
  ...
  const block = (0, _blocks.createBlock)(defaultBlockName, attributes);
